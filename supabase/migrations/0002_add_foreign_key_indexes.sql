-- Index foreign-key columns used by genealogy joins.
create index if not exists storyboards_script_id_idx on public.storyboards(script_id);
create index if not exists storyboard_shots_storyboard_id_idx on public.storyboard_shots(storyboard_id);
