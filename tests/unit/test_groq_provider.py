...
from tiktok_factory.agents.schemas import DirectorOutput, QAAgentOutput, ViralJudgeOutput
from tiktok_factory.providers.groq import (
    GroqProvider,
...
VALID = {
    "enriched_concept": "A richer city concept",
...
def client(handler):
    return httpx.Client(
        transport=httpx.MockTransport(handler), base_url="https://api.groq.test"
    )
...
def test_structured_output_and_strict_schema_request():
    ...
    result = GroqProvider("not-a-real-secret", client=client(handler)).structured(
        "director", "seed", DirectorOutput, "model"
    )
    assert result.enriched_concept == VALID["enriched_concept"]
    assert seen["response_format"]["json_schema"]["strict"] is True


def test_live_strict_schemas_require_every_property():
    for schema_type in (ViralJudgeOutput, QAAgentOutput):
...
def test_timeout_retries_are_bounded():
    ...
    provider = GroqProvider(
        "fake", client=client(handler), max_retries=2, sleep=lambda delay: None
    )
