...
def build_intelligent_pipeline(mode: str) -> IntelligentPipeline:
    media = FactoryPipeline(SyntheticVideoProvider())
    if mode == "mock":
    ...
    return IntelligentPipeline(llm, media, repository,
...
def main() -> int:
    ...
    intelligent.add_argument("--idea", default="A futuristic city where gravity reverses for exactly one minute every midnight")
    ...
    if args.command == "intelligent-demo":
        try:
            pipeline = build_intelligent_pipeline(args.mode)
            result = pipeline.run(args.idea, Path(args.output), args.correlation_id)
        except RuntimeError as exc:
            print(f"error: {exc}")
            return 2
        print(json.dumps({"status": result.status, "idea_id": str(result.idea.id),
                          "viral_score": result.viral_score.total,
                          "llm_calls": len(pipeline.agent_runs), "video": str(result.video.path)}))
        return 0
