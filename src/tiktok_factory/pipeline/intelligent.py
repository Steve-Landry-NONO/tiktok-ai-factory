"""Agent-driven V2/V3 orchestration layered on the tested media pipeline."""
import json
...
from tiktok_factory.agents.schemas import CreativeProducerOutput, DirectorOutput, ViralJudgeOutput
...
class IntelligentPipeline:
    def __init__(
        ...
        media_pipeline: FactoryPipeline,
    ...
    def run(self, seed: str, output_dir: Path, correlation_id: str | None = None) -> PipelineResult:
        ...
        creative = self._call(
            "creative_producer",
            self._creative_prompt(director),
            CreativeProducerOutput,
            self.primary_model,
        )
        ...
        idea = ContentIdea(
            id=idea_id,
            concept=director.enriched_concept,
            source="intelligent_pipeline",
            creator="director",
        )
        shots = [
            StoryboardShot(
                id=uuid5(NAMESPACE_URL, f"{correlation_id}:shot:{number}"),
                number=number,
                concept=f"{director.creative_direction}. {shot.concept}",
                caption=shot.caption,
                duration_seconds=shot.duration_seconds,
            )
            for number, shot in enumerate(creative.shots, 1)
        ]
        result = self.media_pipeline.run(
            director.enriched_concept,
            output_dir,
            idea=idea,
            viral_score=score,
            script_content=(creative.hook, creative.narration, creative.call_to_action),
            prepared_shots=shots,
            script_id=script_id,
    ...
    def _call(self, agent: str, prompt: str, schema: type[T], model: str) -> T:
        ...
        if self.repository:
            self.repository.save_agent_run(run, model)
        return output
    ...
    def _creative_prompt(director: DirectorOutput) -> str:
        return (
            "Create a short vertical-video script with exactly 3 or 4 visually distinct shots. "
            "Each shot must be 2-10 seconds, start with immediate visual action, avoid embedded text, "
            "and preserve a consistent world/style across shots. Build a strong opening hook and a "
            "loop-friendly ending from: "
...
class MockIntelligentLLM:
    ...
    def structured(self, agent: str, prompt: str, schema: type[T], model: str) -> T:
        ...
        if schema is DirectorOutput:
            data: dict[str, Any] = {
                "enriched_concept": "A futuristic city where gravity reverses for one minute every midnight",
            ...
            data = {
                "hook": "At midnight, this entire city falls upward.",
                ...
                "shots": [
                    {
                        "concept": "Neon skyline moments before midnight",
                        "caption": "11:59:59",
                        "duration_seconds": 2,
                    },
                    {
                        "concept": "People and objects drift upward",
                        "caption": "Gravity reversed",
                        "duration_seconds": 2,
                    },
                    {
                        "concept": "Everything lands as the clock resets",
